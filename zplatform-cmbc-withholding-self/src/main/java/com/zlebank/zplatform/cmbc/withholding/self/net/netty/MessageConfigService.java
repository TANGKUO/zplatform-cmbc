...
public class MessageConfigService {
	...
	private void refresh() {
		...
		if (hostAddress == null) {
			configParams.put("HOST_ADDRESS", Constant.getInstance().getCmbc_self_withholding_ip());
		}
		
		String hostPort = StringUtils.trimToNull((String) configParams.get("HOST_PORT"));// 主机端口
		if (hostPort == null) {
			configParams.put("HOST_PORT",Constant.getInstance().getCmbc_self_withholding_port()+"" );
		}
		...
		if (companyCode == null) {
			...
			configParams.put("COMPANY_CODE", Constant.getInstance().getCmbc_self_merid());
		}
		String tranMessageCode = StringUtils.trimToNull((String) configParams.get("MESSAGE_CODE_TRAN"));// 报文码:代付交易
		if (tranMessageCode == null) {
			configParams.put("MESSAGE_CODE_TRAN", Constant.WITHHOLDINGSELF);
		}
		...
		if (queryMessageCode == null) {
			configParams.put("MESSAGE_CODE_TRAN_QUERY", Constant.WITHHOLDINGSELFQUERY);
		}
