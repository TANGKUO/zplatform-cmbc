package com.zlebank.zplatform.cmbc.withholding.self.net.netty;
...
import com.zlebank.zplatform.cmbc.withholding.self.request.bean.RealTimeSelfWithholdingQueryBean;
...
public class MessageHandler {
	...
	public byte[] pack(RealTimeSelfWithholdingBean realTimeWithholdingBean ) {
		byte[] bytes = null;
		try {
			...
			String privateKey = Constant.getInstance().getCmbc_withholding_self_private_key();
			String publicKey = Constant.getInstance().getCmbc_withholding_self_public_key();
			
			String messageCode = Constant.WITHHOLDINGSELF;
			String xml = realTimeWithholdingBean.toXML();//realTimePayBean.toXML();
	...
	//
	public byte[] pack(RealTimeSelfWithholdingQueryBean realNameAuthQueryBean) {
		byte[] bytes = null;
		try {
			String charset = messageConfigService.getString("CHARSET");// 字符集
			int headLength = messageConfigService.getInt("HEAD_LENGTH", 8);// 报文头长度
			...
			String messageCode = Constant.WITHHOLDINGSELFQUERY;
			String xml = realNameAuthQueryBean.toXML();//realTimePayBean.toXML();
			logger.info("本地--->对端明文{}:{}", new Object[] { messageCode, xml });
			byte[] xmlBytes = xml.getBytes(charset);
	...
	public Map<String, Object> unpack(byte[] bytes) {
		...
		try {
			...
			String privateKey = Constant.getInstance().getCmbc_withholding_self_private_key();// 合作方私钥
			String publicKey = Constant.getInstance().getCmbc_withholding_self_public_key();// 银行公钥
			...
			if(Constant.WITHHOLDINGSELF.equals(messageCode)){
			...
			}else if(Constant.WITHHOLDINGSELFQUERY.equals(messageCode)){
			...
			}else if(Constant.WITHHOLDINGSELF_REALNAME.equals(messageCode)){
	            ...
	            resultMap.put("result", queryResultBean);*/
			}else if(Constant.WITHHOLDINGSELF_REALNAMEQUERY.equals(messageCode)){
